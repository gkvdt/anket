    ...
    <form action="" method="post" name="edit_user">
        <div class="col-lg-4 col-md-6">
            <div class="form-group">
                <input type="text" class="form-control" placeholder="User E-Mail" name="admin_username"/><br>
      ...
      <?php
       ...
       if (post('submit')){
            ...
            $id = get('id');
           $mail = post('admin_username');
           $password = post('admin_password');

           if ($mail){
               $query = $db->prepare("UPDATE admin SET admin_username =? WHERE admin_id=?");
           ...
           if ($password){
               $query = $db->prepare("UPDATE admin SET  admin_password=? WHERE user_id=?");
