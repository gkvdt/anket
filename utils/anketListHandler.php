...
function renderAnket($anket)
{
    ...
    <tr onclick="window.location.href = 'anket-detay.php?anket_id=<?php echo $anket['id']; ?>'"
        style="cursor: pointer;"> <!-- edit link and add id -->
        <?php // TODO anketleri listele
        ?>
        <td><?php echo $anket['baslik']; ?></td>
        <td><?php echo $count; ?></td>
        <td><?php activeStatus($anket['id']); ?></td>
        <td><?php removeAnket($anket['id']); ?></td>

    </tr>
    <?php
}

function removeAnket($id)
{
	    ?>
    <a href="../utils/deleteAnket.php?anket_id=<?php echo $id;?>" class="btn btn-danger">Delete</a>
    <?php
}
...
{
    ?>
    <p class="btn btn-success"> Actived</p>
    <?php
}
